...
export class HttpService {
  ...
  constructor( private http: Http ) {
    this.headerInfo = new Headers( { 'Content-Type': 'application/json' , 'Access-Control-Allow-Origin' : '*' } );
  ...
  sendDelete( uri: string ): Observable<any> {
    return this.http.delete( this.defaultUrl + uri, this.reqOptions )
      .map( response => response.json() )
      .catch( this.handleError );
  }
